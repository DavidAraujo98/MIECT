...
void *Thread_1_code(void *arg)
{
	...
	tp.tv_nsec = args->period * 1000000;
	tp.tv_sec = PERIOD_S;
...
int main(int argc, char *argv[])
{
	...
	if (argc != 4)
	{
		printf("Usage: %s PROCNAME PRIORITY, where PROCNAME is a string, PRIORITY an integer and PERIODICITY also an integer\n\r", argv[0]);
		return -1;
	}
	...
	parm.sched_priority = atoi(argv[2]);	/* Thread priority */
	pthread_attr_setschedparam(&attr, &parm);

	struct arg_struct args;
	args.name = &procname[0];
	args.period = atoi(argv[3]);
	if (args.period < 50 || args.period > 500)
	{
		printf("\n\r Error creating Thread: Period must be between 50 and 500\n");
		return -1;
	}

	/* Create periodic thread/task */
	err = pthread_create(&threadid, NULL /*&attr*/, Thread_1_code, (void *)&args);
	if (err != 0)
	{
