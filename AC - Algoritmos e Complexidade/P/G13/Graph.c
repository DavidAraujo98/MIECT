...
Graph* GraphCopy(const Graph* g) {
  ...
  aux->numEdges = g->numEdges;
  aux->verticesList = g->verticesList;
  aux->isComplete = g->isComplete;
  return aux;
}
...
Graph* GraphFromFile(FILE* f) {
  ...
  int num_edges = 0;
  fscanf(f, "%d", &num_edges);
  //printf("num_edges -> : %d\n", num_edges);

  Graph *g = GraphCreate(num_vertices, digraph, wheighted);
  ...
  while (fgets(buffer, sizeof(buffer), f) != NULL)
  {
    ...
    if (ptr != NULL)
    {
      ...
      if (edge1 != edge2)
      {
        ...
        {
          int weight = atoi(ptr);
          GraphAddWeightedEdge(g, edge1, edge2, weight);
        }
        else
        {
          GraphAddEdge(g, edge1, edge2);
        }
      }
    }
    //printf("%s\n", ptr);
