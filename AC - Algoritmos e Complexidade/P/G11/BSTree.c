// NMEC: 93444
// NOME: David José Araújo Ferreira
//
...
// A: To delete the tree yes, because it starts at the bottom deleting every node in that level before retreating
// to the privious level, and is able to do this without storing the node pointers in an auxiliary pointer
...
static int _treeGetHeightREC(const struct _BSTreeNode *root)
{
  ...
  if (root != NULL)
  {
    ...
    if (root->left != NULL)
    {
      l = _treeGetHeightREC(root->left);
    }
    if (root->right != NULL)
    {
      r = _treeGetHeightREC(root->right);
    }
    f = (r >= l ? r : l) + 1;
  }
