...
cidades_portugal_as_tuple = (lambda s: func_actions(road_connections, s),
                             ...
                             lambda s, goal: func_heuristic(
                                 city_coordinates, s, goal),
...
print('breadth', t.optimize, t.search2())
print(t.non_terminals, t.terminals)  # Still wrong
print(t.solution)
print(t.open_nodes)
print("Time:", time.process_time()-t0)

# Ex5.1 - INCOMPLETE - non_terminal counter is 21 instead of 18
...
# Ex6.2 - INCOMPLETE - Error propagated from Ex5
...
initial_state += [Floor('x'), On('y', 'x'), On('z', 'y'),
                  On('w', 'z'), Free('w')]
...
plan = [Stack.instanciate(['e', 'b']), Unstack.instanciate(['e', 'b']), Stack.instanciate(['e', 'c']),
        Pickup.instanciate(['a']), Stack.instanciate(
    ['a', 'b']), Unstack.instanciate(['e', 'c']),
    Stack.instanciate(['e', 'w']), Unstack.instanciate(
            ['c', 'd']), Stack.instanciate(['c', 'd']),
    Unstack.instanciate(['a', 'b']), Stack.instanciate(
    ['a', 'e']), Pickup.instanciate(['b']),
