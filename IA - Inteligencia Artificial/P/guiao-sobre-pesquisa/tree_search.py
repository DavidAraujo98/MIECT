...
class SearchTree:
    ...
    def __init__(self, problem, strategy='breadth'):
        ...
        root = SearchNode(problem.initial, None, 0, 0, problem.domain.heuristic(
    ...
    def search(self, limit=None):
        while self.open_nodes != []:
            ...
            if self.problem.goal_test(node.state):
                ...
                self.cost = node.cost
                self.average_depth = (sum(n.depth for n in self.open_nodes)+node.depth) / (len(self.open_nodes)+1)
                self.avg_branching = (
                    (self.terminals + self.non_terminals) - 1) / self.non_terminals
                nplan=[]
                for os, ns in zip (self.get_path(node), self.get_path(node)[1:]):
                    for a in self.plan:
                        if ns == self.problem.domain.result(os, a):
                            nplan.append(a)
                            break
                self.plan = nplan
            ...
            for a in self.problem.domain.actions(node.state):
                ...
                newnode = SearchNode(newstate, node, node.depth + 1, node.cost + self.problem.domain.cost(
